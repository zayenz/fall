...
use crate::{AstNode, Node, NodeType};
...
pub fn process_subtree_bottom_up<'f, V, C>(
    node: Node<'f>,
    mut visitor: VisitorBuilder<'f, C, V>,
) -> C
where
    V: Visit<'f, Context = C>,
{
...
impl<'f, C, V> VisitorBuilder<'f, C, V> {
    ...
    pub fn visit_nodes<'n, F>(
        self,
        nodes: &'n [NodeType],
        f: F,
    ) -> VisitorBuilder<'f, C, TyVisitor<'n, V, F>>
    where
        V: Visit<'f>,
